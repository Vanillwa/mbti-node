...
router.get("/api/chat", async (req, res) => {
  ...
  where (userId1 = ${req.user.userId} and user1Status = "ok" ) or (userId2 = ${req.user.userId} and user2Status = "ok" )
...
//채팅방 입장
router.get("/api/chat/join/:roomId", async (req, res) => {
...
// 채팅방 나가기
router.get("/api/chat/quit/:roomId", async (req, res) => {
  const { roomId } = req.params;
